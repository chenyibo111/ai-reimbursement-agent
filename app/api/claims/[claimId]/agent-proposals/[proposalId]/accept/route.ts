...
export async function POST(request: Request, context: { params: Promise<{ claimId: string; proposalId: string }> }) {
  try {
    ...
    const result = await resolveAgentProposal({ actorId, claimId, proposalId, action: "ACCEPT", expectedVersion: body.data.expectedVersion }, { resolve: (input) => new PrismaClaimRepository(getPrisma()).resolveAgentProposal(input) });
