...
export function ClaimChat({ claimId, version, proposals, onComplete }: Props) {
  ...
  const { upload, message: uploadMessage, isUploading } = useReceiptUpload({ claimId, onComplete: onComplete });
  ...
  async function resolve(proposal: AgentProposal, action: "accept" | "reject") {
    ...
    try { const response = await fetch(`/api/claims/${claimId}/agent-proposals/${proposal.id}/${action}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ expectedVersion: version }) }); const payload = await response.json() as { error?: string }; if (!response.ok) { if (response.status === 409) { onComplete(); throw new Error("草稿已更新，相关建议已刷新，请按最新内容处理。"); } throw new Error(payload.error || "无法处理该建议。"); } onComplete(); }
