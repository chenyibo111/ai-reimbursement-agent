...
type StoredExpenseItem = { id: string; receiptId: string | null; amountCents: number; invoiceNumber: string | null; issuedOn: string | null | Date };
...
export function buildAgentContext(input: { claim: { id?: string; version: number; purpose: string | null; totalAmountCents: number; expenseItems: StoredExpenseItem[]; receipts: StoredReceipt[] }; issues: ValidationIssue[] }): AgentContext {
  ...
  const expenses = input.claim.expenseItems.map((item, index) => {
    ...
    return { target, amountCents: item.amountCents, invoiceNumber: item.invoiceNumber, issuedOn: item.issuedOn ? toDateString(item.issuedOn) : null, receipt: item.receiptId ? receipts.get(item.receiptId) : undefined };
  });
  ...
  for (const expense of expenses) {
    ...
    if (hasIssue(input.issues, "CONFIRM_INVOICE_NUMBER") && isLowConfidence(expense.receipt?.extractionPayload, "invoiceNumber")) fields.push("invoiceNumber");
    if (hasIssue(input.issues, "CONFIRM_ISSUED_ON") && isLowConfidence(expense.receipt?.extractionPayload, "issuedOn")) fields.push("issuedOn");
    if (hasIssue(input.issues, "CONFIRM_TOTAL_AMOUNT") && isLowConfidence(expense.receipt?.extractionPayload, "totalAmountCents")) fields.push("totalAmountCents");
    if (fields.length) allowed.set(expense.target, fields);
  }
  return { summary: { purpose: input.claim.purpose, totalAmountCents: input.claim.totalAmountCents, expenses: expenses.map(({ receipt: _receipt, ...expense }) => expense) }, issues: input.issues, allowedTargets: [...allowed.entries()].map(([target, fields]) => ({ target, fields })), targetMap, claimVersion: input.claim.version };
}
